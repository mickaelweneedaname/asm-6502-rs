...
pub fn get_instruction(name: &str, mode: Mode) -> &Instruction {
    let up_name = name.to_uppercase();
    let infered_mode = if ["JMP", "JSR"].contains(&up_name.as_str()) && mode == Mode::Relative {
        Mode::Absolute
    } else {
        mode
    };
    INSTRUCTIONS
        .iter()
        .find(|i| i.name == up_name && i.mode == infered_mode)
        .unwrap_or_else(|| {
            panic!(
                "get_instruction :: instruction not found : {name} : {:?})",
                infered_mode
            )
        })
}
...
#[cfg(test)]
mod tests {
    use super::{Mode, INSTRUCTION_MAP};
    ...
    fn all_modes_and_sizes_are_coherent() {
        let bad_instructions = INSTRUCTION_MAP
            .iter()
            .map(|(_, i)| i)
            .filter(|i| i.len != get_lenght_from_mode(&i.mode));
        assert_eq!(0, bad_instructions.count())
    }
}
